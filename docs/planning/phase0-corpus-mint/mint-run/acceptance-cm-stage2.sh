#!/usr/bin/env bash
# STAGE 2 — CTL-2 + CTL-3 + 8 fresh real (10 records), controls FIRST. Frozen 2026-09-19
# BEFORE it was run: THIS FILE CONTAINS NO RESULT. The run happens once; stdout is
# committed verbatim to acceptance-cm-stage2.out, whatever it says (Rule D,
...
uv run belay phase0 run "$ROOT/batch" \
  --ledger "$HOLDER/runs/cm-stage2.json" \
