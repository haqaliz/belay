#!/usr/bin/env bash
# STAGE 1 — probe (CTL-1 + CTL-4, 2 records). Frozen 2026-09-19 BEFORE it was run:
# THIS FILE CONTAINS NO RESULT. The run happens once; stdout is committed verbatim to
# acceptance-cm-stage1.out, whatever it says (Rule D, phase0-mint-run/prd.md:97-101).
#
...
uv run belay phase0 run "$ROOT/batch" \
  --ledger "$HOLDER/runs/cm-stage1.json" \
