...
- [x] **L1 ✅** — the Phase-0 number is published: ≥50 instances, ≥3 independent
      hand-audited TPs, FP rate stated, no INSTRUMENT SUSPECT, ledger re-renderable
      via `belay phase0 report`. **Box ticked 2026-09-05 as bookkeeping**, not as a new
      decision: L1's own section has read DONE since 2026-08-12 (n=60, 11 hand-audited
      TPs, 0 adjudicated FPs of 23 trajectory FAILs, 4/4 controls clean, ledgers
      committed and byte-identically re-renderable). Quote **11/60 = 18.3%**, never the
      raw 71.2% without its decomposition.
...
|------|-----------------|--------|------------------|
| 2026-09-23 | `corpus-mint-second-run` | Block 0 | ✅ **DONE — the composite-snapshot fix lands and the third probe clears its gate; the corpus grows for the first time since 2026-08-12.** Not a gate run, no Phase-0 number (n = 8 real). Run 2 stopped (`INSTRUMENT SUSPECT`), and the measured cause was a composite-transport correlation artifact (`annotation_for_turn` read only `live[-1]`). The fix reads the latest `tools/list` with its broadcast twins (exact, never a time window; a disagreement gives `tool-ambiguous` → UNVERIFIED). Run 3: stage 1 VERIFIED_CLEAN 2; stage 2 10/10 captured, 8 CLEAN / 2 trajectory FAIL (`django-11422`, `django-14382`), no control FAILed. Both banked `pending`, `corpus run` 9/9 MATCH, hand-replay MATCH. Run-1 root cause corrected by measurement. Coverage gain, not improved detection; **no published number moves**; 2743 → 2764 tests. **Open, owner's:** label the 2 pending cases. See `docs/planning/phase0-corpus-mint/audit-and-publish/`. |
| 2026-09-22 | `calibration-ledger` | Block 0 | ⏸️ **BUILT, NOT MERGED — C10 slice 2 lands on `feat/calibration-ledger/aliz`, full suite green (2743 passed / 0 failed).** `belay triage-ledger <verify-json>` — a pure re-render measurement pairing each triaged turn's score with the verdict replay produced: reliability curve (equal-width deciles), ECE, and violations-skipped × budget-saved sweeps; the named v0.36.0 follow-up ships too (budgeted-mode `triage` section carries skipped turns' scores, `"skipped": true`, shadow byte-unchanged — the on/off identity refutation passes unmodified). **Honesty lines:** UNVERIFIED excluded with the count stated; zero decided rows ⇒ named refusal, no rates, exit 0 (the INSTRUMENT SUSPECT shape); the demo capture proves the mechanics but contributes zero violation rows — the first real positive rows come from the mint's second run (the owner's S-1 decision); **no published number moves** (`11/60 = 18.3%`, `precision 0.00`, `1/15`, `4/16`, `recall 0.00` stand unedited); 2721 → 2743 tests. **NOT built, by name:** multi-document aggregation, budget auto-tuning. See `docs/planning/calibration-ledger/`. |
| 2026-09-21 | `jev-triage` | Block 0 | ⏸️ **BUILT, NOT MERGED — the C10 slice-1 triage seam lands on `feat/jev-triage/aliz`, full suite green (2714 passed / 0 failed / 25 named-caused skips).** Provider-neutral seam (owner PS: any model = any command; Jev = first reference author, laya later = another author, never an engine adapter): `Triage`/`NullTriage`/`SubprocessTriage` + whitelisted-derived-features payload (raw content structurally unrepresentable), budget machinery (threshold ∪ top-N, shadow default, fail-open on command failure — the review-gate amendment), `verify`-only flags (`--triage-author`/`--triage-threshold`/`--triage-top-n`/`--no-triage`, parity guard registered), skipped turns UNVERIFIED-by-budget (`"skipped by the triage budget"`, closed-vocabulary guard), additive `triage` JSON section (absent-never-zero), the Jev REST reference author (`BELAY_JEV_KEY` author-only, engine never reads/passes a key — pinned), and the refutation: on/off verdicts byte-identical with an anti-vacuity spy, absent ⇒ no subprocess/no network. **Honesty lines:** banks no calibration ledger and saves no cost yet (shadow default); triage never a verdict; **no published number moves** (`11/60 = 18.3%`, `precision 0.00`, `1/15`, `4/16`, `recall 0.00` stand unedited); 2575 → 2714 tests. **Open, owner's:** the real Jev REST contract before the manual live test runs; the calibration-ledger slice is downstream of the S-1 mint decision. See `docs/planning/jev-triage/`. |
| 2026-09-19 | `phase0-corpus-mint` | Block 0 | ⏸️ **STOPPED AT ITS OWN GATE — not DONE, and not a failure to hide.** Two aspects shipped: **`a3-author`** (the A3 claim axis becomes drivable — `run_verify` called `run_batch()` without `claim_author=`, so the mint's `--verify` could NEVER fill the claim column whatever the env said while the printed CLI command could, and `eval/README.md:727-729` called them equivalent; fixed in +8 lines inside the lazy-import block. Reference author ships at `src/belay/verify/reference_claim_author.py` — NOT `authoring/`, which declares it holds nothing in the verdict path while A3 may downgrade a turn. Stdlib only, `--tools ""` + `--strict-mcp-config`, `ANTHROPIC_*` scrubbed by absence. **Live at n=1**: author ran (observed on disk), check EXECUTED, exit 0 — D3 silence; A3 did not manufacture intent drift on the honest negative control) and **`mint-registry`** (derived-never-transcribed exclusion set, byte-identical regeneration, 8 reals colliding with no prior registry or ledger). **`mint-run` ran ONCE under the freeze protocol and its gate STOPPED it**: stage 1 minted 2 captured / 0 failed, then verified to `NO_VERIFIABLE_TURNS: 2`, **INSTRUMENT SUSPECT**, UNVERIFIED 3/3 = 100% — a pre-registered STOP, so stage 2 never launched (cost: 2 controls, 67.5s, 5 requests, not 12 instances). **NOT a D-3 void** (no control FAILed), **not a result about agents** (the mint half succeeded), **not a zero** (INSTRUMENT SUSPECT refuses to print a rate). **Causes are PRE-EXISTING, proven from the committed gate ledgers**: the 2026-08-12 run that PROCEEDed carries `UNRESTORABLE_SNAPSHOT_FAILED` 16/122 and `replayed but effect unverified` 8 — it absorbed them across hundreds of turns; a 3-turn probe cannot. **Root cause measured**: the pinned npm filesystem server declares NO annotations, so effect-conformance abstains by its own rule — honest, not broken, but it means **a corpus-filling mint can never bank a per-turn case against annotation-less servers**. Two corrections recorded: the missing `phase0 run --claim-author` is a PINNED DECISION not a parity defect, and "30 fresh" is never-DRAWN (never-captured is 83). Substrate repair: 3 missing eval symlinks left **1,344** recorded paths dead; restored and proven by `corpus run` 7/7 MATCH + `s1p` 0/11 UNVERIFIED. **No published number moves**; 2540 → 2575 tests. Open, owner's by S-1: fix the instrument / re-scope the probe / declare a second run / stop — recommendation **fix the instrument**. |
| 2026-09-15 | `invariant-authoring-experiment` | Block 0 | ✅ DONE. R3's third mitigation, not a checklist item: **a model writes A1 policy from a task spec, execution calibrates it, and an uncalibrated or tampered artifact can never FAIL.** `belay invariant infer` runs an out-of-process BYOK author (JSON-in/JSON-out — the engine never calls a model), validates against the known rule vocabulary, calibrates by replaying the control once through the shipped `verify_turn` composition (`CALIBRATION_FAILED` for an over-firing candidate, `CONTROL_UNREPLAYABLE` for a vacuous control), and emits a reviewable `belay-authored-invariants/1` artifact with a sha256 over the normalized policy set + task + control. `--invariants <artifact>` loads it additively by shape (a JSON list stays operator policy, byte-unchanged) through `parse_authored_invariants` — the deliberate fourth policy producer admitted by name in the provenance guard: calibrated → enforced like operator policy; uncalibrated → `AUTHORED_INVARIANT_UNCALIBRATED`; edited after calibration → `AUTHORED_INVARIANT_ALTERED`; both UNVERIFIED on every turn, never FAIL, never a silent PASS/skip. Reference `claude -p` author ships (`--tools ""` + `--strict-mcp-config`, env scrubbed by absence); the live proof is `manual`-marked and owner-run. **Honesty lines:** dark by default; A1 semantics untouched (no new axis/status/default); calibration proves *does not fire on this control*, nothing more; **no published number moves** — `11/60 = 18.3%`, `precision 0.00`, `1/15`, `4/16`, `recall 0.00` stand unedited. **NOT built, by name:** the authoring UI / per-repo library management, live-model CI, per-invariant digests, `--control` as a banked case, artifact re-calibration. 2403 → 2540 tests. |
| 2026-09-15 | `approval-gate` | Block 0 | ✅ DONE. **Phase 2's second goal — the approval gate — ships** (aspects `decision-model` → `hold-channel` → `proxy-deny` → `trace-observations` → `verify-report`), not a checklist item: `BELAY_APPROVAL_DIR` + `BELAY_APPROVAL_TIMEOUT` hold a `tools/call` whose tool declares `destructiveHint`/`openWorldHint` **declared-true** (tri-state — a default is never a declaration) pending human approval through a control-directory file contract (requests/ + decisions/, atomic both sides); approve forwards the frame verbatim, deny returns a named JSON-RPC refusal with the request's own id, the deadline is fail-closed (`APPROVAL_TIMEOUT`; shutdown `APPROVAL_SHUTDOWN`; gate fault `APPROVAL_FAULT` — `decide_c2s` is total). Engine: `BeforeFrame` gains the suppress outcome, the observer sees exactly the delivered stream (cross-chunk suppressed frames; mismatch = named capture error), client writes are frame-atomic under a shared peer lock, approval runs before the turn gate (a denied call never consumes a snapshot), holds key on tool-call + name never request-id (MRTR retries carry new ids). Trace: additive `approval_hold`/`approval_decision` kinds, never frame records — a denied trace correlates with zero `response-without-request`; `belay verify` gains an additive `approval` section (absent-never-zero, verdicts orthogonal to A1/A2/A3). **Honesty lines:** a containment surface, not a verdict axis — a denied call is an observation, never a turn and never a verdict; `11/60 = 18.3%`, `precision 0.00`, `1/15`, `4/16` stand unedited; the gate is a supplement for honest-but-buggy servers, not an adversarial control. **NOT built, by name:** console approval UI/wiring, corpus banking of held/denied attempts (owner decision 2026-09-14), HTTP listeners, `run_process` special-casing, batch-frame holds. 2137 → 2403 tests. |
| 2026-09-13 | `ci-regression-gate` | Block 0 | ✅ DONE. **Phase 2's first goal — the CI regression gate — ships end-to-end** (aspects `run-identity` → `baseline-bank` → `gate-check` → `divergence-banking` → `surface-docs`), not a checklist item: `belay gate baseline` banks a self-contained, identity-keyed baseline (trace + manifests + snapshots + resolved policy + expected verdict set) under the gitignored `baselines/local/<run-id>/`; `belay gate check` re-verifies the baseline under the **stored** policy (no `--invariants`/`--replays`/`--timeout` on check — the banked policy decides, re-bank for a different one) and diffs by the pure decision table: exit 1 regression = a PASS/WARN dimension moves to FAIL (per turn / trajectory / claim / new failing turn); exit 0 named never-failing rows (UNVERIFIED coverage loss, transitions into WARN, fixes, shape, drift); exit 2 preflight = SKIP with named cause, UNVERIFIED never PASS. Run identity via capture-time `BELAY_RUN_ID` (additive `run_identity` trace record, no schema bump) or `--run-id` fallback; regression turns bank as pending corpus cases by default (`--no-ingest` measures without writing). README quickstart step + GitHub Actions snippet + exit-code table, machine-checked by `tests/test_gate_docs.py`; flag-parity guard covers both surfaces (M8). **Honesty lines:** no verdict axis, invariant, coverage line or published number moves — `11/60 = 18.3%`, `precision 0.00`, `1/15`, `4/16` stand unedited; a GitHub Action wrapper and `--strict-shape` are deferred by name. 2188 → 2261+ tests. |
| 2026-09-12 | `invariant-library` | Block 0 | ✅ DONE. R3's library mitigation, not a checklist item: `--invariant-library <name>` (verify / corpus add / phase0 run) + `belay invariant-library list` + the first new A1 rules since C5 — `no-create` / `no-delete`, delta-grounded, byte-prefix scope — plus `tests-read-only` / `source-read-only` presets and an honest unobservable `network-egress` entry (UNVERIFIED-with-cause, never PASS; operator files still reject the name). Two deliberate review-approved guard amendments (producer set + SCAN_AREA). Per-entry fixture corrupt-success cases bank and recompute MATCH (stored-invariants pin: tamper → REGRESSION). No published number moves. 2147 → 2188 tests. |
