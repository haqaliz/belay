...
|------|-----------------|--------|------------------|
| 2026-09-23 | `corpus-mint-second-run` | Block 0 | ⏸️ **STOPPED AT ITS OWN GATE AGAIN — not DONE, not a failure to hide.** The declared second run's stage 1 (2 controls, engine v0.37.0) minted 2/2 and verified to `NO_VERIFIABLE_TURNS: 2`, UNVERIFIED 5/5, `INSTRUMENT SUSPECT` → pre-registered STOP; stage 2 never launched. Not a D-3 void, not a zero, not a gate run. The warrant ("turns now reduce") was refuted: a composite-transport snapshot-correlation artifact (`annotation_for_turn` takes `live[-1]`) keeps filesystem tools at `tool-absent`. `verify-parity` shipped (`--verify` threads the shell server). Zero cases banked, and the 7 existing cases are 7/7 MATCH. Both stage-1 ledgers are committed and re-render byte-identically. **No published number moves**; 2743 → 2755 tests. **Open, owner's by S-1:** continue (fix the correlation, then a third probe) / re-scope / stop. See `docs/planning/phase0-corpus-mint/audit-and-publish/`. |
