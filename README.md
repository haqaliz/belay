...
**The calibration ledger measures whether the triage model's confidence actually predicts violations.** `belay triage-ledger <verify.json>` re-renders a stored `belay verify --json` document as the reliability curve, ECE, and the decision-relevant sweep (at each threshold, violations-skipped × budget-saved) — a **pure re-render: no replay, no re-verification**. The violation column is the per-turn reduced FAIL (UNVERIFIED and budget-skipped turns excluded, their counts stated); a zero-denominator document renders the named `NO_DECIDED_ROWS` refusal with no rates. It never decides the budget — it informs the operator who sets the knobs.
...
**A `PASS` that says exactly what it covers.** `belay verify` re-executes each turn against its restored pre-state. The coverage line travels *with* the status — the network dimension is `NOT_COVERED`, and a rule that judged nothing says so rather than banking its silence as evidence.

<p align="center"><img src="https://raw.githubusercontent.com/haqaliz/belay/master/assets/belay-verify.png" alt="belay verify output for one turn: turn 0 write_file PASS, with A2 replay PASS (the replayed reply reproduced the recorded reply), A2 effect PASS (declared readOnlyHint false, the observed effect conforms), A2 effect:network NOT_COVERED because openWorldHint was declared false but Belay observes no network egress so it is never a PASS and never a fabricated FAIL, and A1 invariant PASS for no-assertion-weakening on tests and testing with an exposure note that zero files were in scope so it carries no information about the rule. The aggregate reads one turn verified, PASS 1, WARN 0, FAIL 0, UNVERIFIED 0." width="760" /></p>

**Every caught failure compounds — and the corpus refuses to flatter itself.** `belay corpus score` grades the engine's own detection against *human* labels; the engine never labels its own cases.

<p align="center"><img src="https://raw.githubusercontent.com/haqaliz/belay/master/assets/belay-corpus.png" alt="belay corpus score output: 7 cases scored against human labels; a confusion matrix of TP 0, FP 0 and FN 1 (hand-adjudicated, n=1); metrics precision n/a because 0 TP and 0 FP is a zero denominator, and recall 0.00 from 0 of 1, not a base rate; a line states that an n/a is a zero denominator, NOT a 1.00, and that the engine never labels its own cases; the corpus holds 0 true positives banked and 7 negative fixtures, turns a human adjudicated false positive." width="760" /></p>

<p align="center"><sub><b>Those are Belay's own figures, not sample output.</b> <code>precision n/a</code> is a <b>zero denominator, not a 1.00</b>: the corpus holds <b>0 true positives</b>, and its 7 cases are turns a human adjudicated <i>false positives</i>, kept as negative fixtures. Recall is <code>0.00</code> (0/1, n=1) — one hand-adjudicated miss, which is not a base rate. See <a href="#what-the-a1-default-does-and-does-not-judge">Coverage &amp; limits</a>.</sub></p>
...
`NOT_COVERED` is a **sub-verdict-only** status, and it is deliberately not one of the four a turn can reduce to. It marks a dimension Belay cannot weigh a verdict on at all — as opposed to `UNVERIFIED`, which means Belay tried and could not. **There are two such dimensions** *(one until 2026-09-21)*: a tool's `openWorldHint: false` network promise, which no filesystem delta can confirm or refute; and **a tool the server declared no `readOnlyHint` for** — there is no contract, so contract-conformance was never in scope for that turn. The three ways Belay can *fail to observe* a contract — no `tools/list` snapshot before the call, the tool absent from the snapshot, an unreadable request frame — remain `UNVERIFIED`, each with its named cause. An absent contract is still never treated as a permissive one: the sub-verdict is never `PASS`, and the boundary is printed on every surface. The reduction **drops it before ranking**, so it never lowers a turn and never lifts one; if nothing scoreable remains the turn is `UNVERIFIED`, never `NOT_COVERED` and never `PASS`. The trade is stated plainly in [Coverage & limits](#a-pass-excludes-the-network-dimension): a `PASS` is a pass *on the dimensions Belay checks*, and every surface that prints a status also prints what fell outside them.
...
>
> **A second boundary landed on 2026-09-21 and the same warning binds it.** A tool the server declared no `readOnlyHint` for used to drag its turn to `UNVERIFIED` too — so against a server that declares no annotations at all (the pinned reference filesystem server declares none) *every* turn read `UNVERIFIED` however well it replayed, and an honest clean run was indistinguishable from a broken instrument. That case is now an `effect` `NOT_COVERED` sub-verdict: there is no contract, so conformance was never in scope. **The `UNVERIFIED` rate across this boundary is likewise not comparable, and the drop is a reclassification, never improved detection** — no published number was recomputed. The honest cost, stated plainly: **it rewards a silent server.** Omitting the annotation now earns a `PASS` where it earned an `UNVERIFIED`, and omitting it is the adversarial move — accepted because this axis catches nothing adversarial by construction and your own declared invariants (A1) remain the load-bearing mechanism. What a declaring server still buys is a verdict with nothing withheld; the coverage line is not suppressible.

### The approval gate holds on declared facts, and nothing else
The optional approval gate (`BELAY_APPROVAL_DIR`, with `BELAY_APPROVAL_TIMEOUT` for the deadline) holds a `tools/call` until a human decides, and records the hold and the decision in the trace (`approval_hold` / `approval_decision`; `belay verify` reports them on both surfaces). It holds on **declared `destructiveHint` / `openWorldHint` facts only** — the server's own self-declared annotations, read from the live `tools/list`. Annotations are *hints*: the MCP spec says they are not guaranteed to describe the tool faithfully, and **a default is never a declaration** — an absent `destructiveHint` is not "declared true", even though the spec's fail-safe default for it is true. So the gate is a **supplement, not an adversarial control**: it gives a human the choice on honest-but-risky declared tools; it catches nothing adversarial (a malicious server omits the annotation, or lies in the safe direction), and a run that never configured the gate is **byte-identical** to one that did with nothing triggering — the gate adds no record, no hold, and no delay.
...
`belay interop correlate <otlp-spans.json> <trace-file>` joins a third-party OTel span to a Belay-recorded MCP turn by the **W3C `traceparent`** the client propagated into MCP `_meta` — the exact string Belay already captured as `trace_context` (C1) — never a time-window or name-based heuristic. A span whose `(traceId, spanId)` names no recorded turn, names more than one (an `ambiguous-correlation`), or was matched but never replayed (no `--server` given, or an unrestorable pre-state) is reported **uncorrelated / `UNVERIFIED`, never `PASS`**, with the exact named cause. Interop is OTLP/JSON parsed with the standard library — **no OpenTelemetry SDK dependency** (zero-dep preserved). This first slice is ingest + correlate + attach over a **single trace file**, and the export
direction ships with it: `belay interop export <otlp> <trace> [--out FILE]` writes verdicts back
into the OTLP document as span attributes/events (a fixture collector — a file, or stdout), with
the coverage line and UNVERIFIED-never-PASS in-band. A **live OTLP exporter** and aggregating a
**directory** of traces remain follow-ups. The correlation rate (`matched/total`, denominator
always shown) measures how much of the agent's recorded activity actually crossed the MCP
boundary — the R6 number.
