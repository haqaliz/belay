...
The limit that creates, stated exactly: **a turn whose tool none of the servers you named offers is not checked against the boundary that actually served it.** Replay still sends the recorded `tools/call`, the server answers that it has no such tool, and that answer reproduces on every re-invoke — so **today Belay reports such a turn as a deterministic result-equivalence `FAIL`**, a confident verdict about a turn nothing verified. That is a defect, not a design: the 2026-08-12 gate mint's **171 per-turn FAILs were all this shape** (hand-verified on `django-12125` turn 8; see [`PHASE0_RESULTS.md`](docs/technical/PHASE0_RESULTS.md)), which is why that run's per-turn FAIL rate is an instrument artifact and never a violation rate. Making it an honest abstention with a named cause instead is a separate unit and **has not landed** (`docs/planning/verify-tool-not-offered/`); until it does, name every server your run used, and read a `FAIL` on a turn whose tool your `--server` does not implement as *"Belay was pointed at the wrong boundary"*, not as a finding about the agent.
