...
This skill supports planning at initiative scope — features, epics, or projects spanning one or more cycles. In the `belay-begin-fast` pipeline it is Phase 4: refine and self-critique the `prd.md` that `prd-interview` produced, then surface the gaps before the review gate.
...
8. **Verdict Honesty & Replay** *(Belay-specific)* — Does the work keep replay deterministic and the verdict grounded in re-execution? Does it name which axis (A1/A2/A3) it affects, and does it have an explicit `UNVERIFIED` path with a named cause rather than a silent pass? Does it harden the sandbox/replay/verify moat and add corpus cases, rather than drifting into an agent framework or a bare LLM judge?
...
Problem Statement → Goals & Success Metrics → Users & Scenarios → Requirements (must/should/nice) → Technical Considerations (incl. replay determinism / verdict impact) → Risks & Mitigations → Out of Scope. The Lightweight Brief collapses these to one-liners; the Full Spec adds Data Model, Trace/Verdict Contracts, and Non-Functional Requirements.

### Step 3 — Generate the PRD

Write it following the chosen depth. Always include: explicit assumptions (labeled as such), open questions that still need answers (don't paper over gaps), and a "Risks & Mitigations" section (never skip this). Reference the roadmap's `R1`..`R12` where a named risk applies, and the capability (`C1`..`C9`) the work belongs to.

### Step 4 — Self-critique

After generating, run the Critique workflow (Workflow 1) against your own output. Flag any 🔴 or 🟡 areas and note them at the end as "Areas to strengthen before sharing." In the `belay-begin-fast` pipeline these flagged gaps are exactly what you present at the ⛔ review gate.
...
├─ Technical ......... {replay/verdict impact in a phrase}
├─ Risks ............. {top risk}
...
6. **Never wave through framework drift** — if a PRD's core value is Belay authoring or orchestrating the agent, flag it against the `CLAUDE.md` wedge before approving.
7. **Never wave through a judge dressed as verification** — if a PRD's verdict rests on a model's opinion rather than re-execution, or lets an A3 check emit `PASS`, or renders `UNVERIFIED` as `PASS`, that's a 🔴 regardless of how good the rest is.
