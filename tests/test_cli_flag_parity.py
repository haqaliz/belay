...
EXPECTED: dict[str, frozenset[str]] = {
    ...
    # belongs here.
    "--shell-server": frozenset({"verify", "phase0 run", "gate baseline"}),
    ...
    # `gate baseline --json` prints the stored baseline document.
    "--json": frozenset({"verify", "interop correlate", "interop export", "gate baseline"}),
    ...
    "--no-claim-axis": frozenset({"verify", "phase0 run", "corpus run", "gate baseline"}),
    ...
    # whose expected set includes a claim needs the author surface.
    "--claim-author": frozenset({"verify", "gate baseline"}),
}
