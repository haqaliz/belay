...
def test_the_declaration_names_the_handle_and_both_bundled_filenames(tmp_path):
    ...
    # when this aspect landed; `recorded-miss` moved it to 3; `corpus-trajectory` to 4;
    # `claim-re-derivation` to 5) -- not a loosening, the same two properties stay pinned.
    assert case.schema_version == CASE_SCHEMA_VERSION == 5
