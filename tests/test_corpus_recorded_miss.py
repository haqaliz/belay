...
def test_schema_version_is_five_and_absent_still_means_one(tmp_path: Path) -> None:
    """The version bump, and the unversioned-means-1 precedent it must not disturb."""
    assert CASE_SCHEMA_VERSION == 5

    case = _full_case()
    write_case(tmp_path, case)
    assert load_case(tmp_path).schema_version == 5
...
def test_corpus_run_reports_the_two_new_outcomes_and_exits_zero(
    ...
    monkeypatch.setattr(
        ...
        lambda _dir, *, shell_server_command=None, disable_claim_axis=False: CorpusRun(
            results=[
...
def test_a_green_run_can_contain_a_miss_that_is_no_longer_missed(
    ...
    monkeypatch.setattr("belay.corpus.run.run_corpus", lambda _dir, *, shell_server_command=None, disable_claim_axis=False: run)
